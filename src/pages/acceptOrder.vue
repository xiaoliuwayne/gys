    ...
    <div>
        ...
        <div class="main-body">
            <van-panel>
                <div slot="header">
                    <van-cell title="上传需要找的面料照片：" size="large" label="请上传要找的完整图案、细节图、尺寸参照图、正反面对比等。最多5张。" >
                        <van-button type="primary" slot="right-icon" size="mini" @click="goHistoryRecord" v-show="true">调取产品库</van-button>
                    </van-cell>
                </div>
                <div>
            ...
            <van-panel class="panel-padding">
                ...
                <van-field
                        v-model.number="form.samplePrice"
                        type="number"
                        label="剪版价："
                        required
                >
                    <span slot="right-icon">
                        <div v-show="moreStore" class="select">
                            <select class="choice" v-on:change="indexSelect(form.cuttingUnit)" v-model.number="form.cuttingUnit">
                ...
                >
                    <span slot="right-icon">
                        <div v-show="moreStore" class="select">
                            <select class="choice" v-on:change="indexSelect(form.largeUnit)" v-model.number="form.largeUnit">
                                <option v-for="(item,ii) in columns" :key="ii" v-bind:value="item.indexId">{{item.name}}</option>
                            </select>
                        </div>
                    </span>
...
<script>
    ...
    import {insertInquiryReceipt, uploadImgs, updateInquiryReceiptInfo, queryInquiryReceipt,uploadImage} from '../axios/api'
    ...
    export default {
        ...
        data(){
            return{
                ...
                unit:UNIT,
                moreStore:true,
                columns: [{'indexId':1,'name':'元/米'}, {'indexId':2,'name':'元/公斤'}, {'indexId':3,'name':'元/码'}],
        ...
        watch:{
            curImgsList(oldVal,curVal){
                if(this.curImgsList.length > 5){ //只取前五张
                    this.curImgsList = this.curImgsList.slice(0,5)
                }
            },
            tmpImgUrls(oldVal,curVal){
                if(this.tmpImgUrls.length > 5){ //只取前五张
                    this.tmpImgUrls = this.tmpImgUrls.slice(0,5)
                }
            }
        ...
        created(){
          ...
          this.orderStatusKey = this.$route.query.orderStatusKey;
           this.receiptId = this.$route.query.receiptId ? this.$route.query.receiptId : 0;
           let itemReceiptId = this.$route.query.itemReceiptId ? this.$route.query.itemReceiptId : 0; //产品库过来的
           let receiptId = (itemReceiptId && itemReceiptId>0) ? itemReceiptId : this.receiptId;  //不是产品库来的就是修改回单的
            if(receiptId > 0){
                let params = {'receiptId': receiptId};
          ...
          if(sessionStorage.getItem('inputInfo')){ // 缓存
        ...
        methods:{
            ...
            async customAction(file, component) { //异步传图到服务器
                this.loadingStatus = true;
                uploadImage(file.file).then(res => {
                    // console.log('customAction=>uploadImage=》res',res);
                    if (res.desc === 'success') {
                        // 原本大图
                        this.tmpImgUrls.push(res.urls[0].image);
                        // 缩略图片
                        let smallImgUrl = getPicUrl(res.urls[0].image,1);
            ...
            inputFile(newFile, oldFile) {
                ...
                if (newFile && oldFile) {  // 更新文件
                    // 开始上传
                    if (newFile.active !== oldFile.active) {
                        console.log('Start upload', newFile.active);
                        // 限定最小字节
            ...
            submit(){
                ...
                console.log('submit=>params',this.form);
            ...
            updateReceipt(){ // 更新回单信息
                ...
                updateInquiryReceiptInfo(formdata).then(res => {
                    ...
                    this.$router.push({name:'orderDetail',query:{'isAccept':'y','inquiryId':this.inquiryId,'receiptId':this.receiptId,'orderStatusKey':this.orderStatusKey}});
                })
            },
            goHistoryRecord(){
                // this.$router.push({name:'historyRecord'});
                // let mark = this.receiptId > 0 ? 'modify': 'create';
                this.$router.push({name:'historyRecord',query:{
                        'inquiryId': this.inquiryId,
                        'receiptId': this.receiptId,
                        // 'mark': mark
                }});
